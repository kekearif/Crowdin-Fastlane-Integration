Simply add this lane to the `Fastfile` and configure the add the `Crowdinfile.yml` to the `fastlane` dir in your project.
...
```ruby
  ...
  lane :strings do
    ...
    path = File.expand_path('..') + "/Snapask/Strings/"
    langs = config["langs"]
